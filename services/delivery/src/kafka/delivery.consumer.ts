...
export class DeliveryConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.consumer = this.kafka.consumer({
      groupId: process.env.KAFKA_GROUP_ID || 'delivery-group',
    });
  ...
  private async handleOrderCreated(order: any) {
    this.logger.log(`Creating delivery for order: ${order.id}`);
    // Aqui você implementaria a lógica de criar entrega
  }
}
